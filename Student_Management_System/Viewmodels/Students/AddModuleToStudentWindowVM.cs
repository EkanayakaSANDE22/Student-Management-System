...
namespace Student_Management_System.Viewmodels.Students
{
    public partial class AddModuleToStudentWindowVM : ObservableObject
    {
        ...
        [ObservableProperty]
        public Module enrolledSelectedModule;
        ...
        public void Delete()
        {
            ...
            {
                var x = MessageBox.Show($"Do you want to delete {EnrolledSelectedModule.Name} ?", "Warning", MessageBoxButton.YesNo, MessageBoxImage.Warning);
                if(x == MessageBoxResult.Yes)
                {
                    using DbDataContext dataContext = new();

                    EnrolledModules module = dataContext.DbEnrolledModules.Single(mod => mod.Id == EnrolledSelectedModule.Mod_Id);
